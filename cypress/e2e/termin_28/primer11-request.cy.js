// Primer za cy.request sa kojim možemo da šaljemo zahtev na server
    ...
    it("Cuvanje odgovora", () => {
        cy.request('https://jsonplaceholder.cypress.io/users?_limit=1')
            ...
            .then(function () {  // da bismo pristupili sacuvanom objektu koristimo "function () { ... }" callback
                ...
                }).its('body').as('objava') // cuvamo kreirani post
            })
            .then(function () {
                console.log(this.objava)
                // kada se pokrene callback .request komande su se izvrsile i mozemo da pristupimo user-u i post-u
                expect(this.objava, 'post has the right user id').property('userId').to.equal(this.user.id)
            })
    })
})
